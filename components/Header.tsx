...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-white/95 backdrop-blur-sm fixed top-0 left-0 right-0 z-50 border-b border-slate-200">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="#home" onClick={(e) => handleNavClick(e, '#home')} className="font-display text-xl font-bold tracking-tight text-slate-900">
          Abem Tadele<span className="text-teal-700">.</span>
        </a>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-md border-b border-slate-200/85 shadow-lg">
          <nav className="flex flex-col items-center space-y-6 py-6">
            {NAV_LINKS.map((link: NavLink) => (
              <a
                ...
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href)}
                className="text-slate-600 hover:text-teal-700 transition-colors duration-300 font-medium text-lg"
              >
                {link.name}
              </a>
            ))}
            <div className="w-2/3 h-px bg-slate-200"></div>
            <div className="flex space-x-6 py-2">
              <a
                href="https://github.com/abemt"
                target="_blank"
                rel="noopener noreferrer"
                className="text-slate-400 hover:text-teal-700 transition-colors duration-300"
                aria-label="GitHub"
              >
